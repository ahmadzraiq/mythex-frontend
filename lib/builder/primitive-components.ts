...
export const PRIMITIVE_COMPONENTS: Record<string, PrimitiveComponent[]> = {
  Layout: [
    { type: 'Box',    label: 'Box',        icon: '□', aiRef: 'Empty vertical layout container. Default frame: full width, no padding or gap. Add set_spacing when padding or gap is needed. Use set_layout / set_display to customise direction and alignment.',
      defaultNode: { type: 'Box',    props: { className: 'flex flex-col w-full' } } },
    { type: 'Box',    label: 'Row',        icon: '⬌', aiRef: 'Empty horizontal layout container. Default frame: full width, small gap, vertically centered children. No padding — add set_spacing when needed. Children are left-aligned by default — use set_layout({justify:"center"}) to center them horizontally.',
      defaultNode: { type: 'Box',    props: { className: 'flex flex-row gap-[8px] w-full items-center' } } },
    { type: 'VStack', label: 'VStack',     icon: '⬇', aiRef: 'Empty vertical stack. Default: gap-[16px] between children, no padding. Call set_spacing to set padding or change the gap. To center children horizontally: set_layout(id, {align:"center"}).',
      defaultNode: { type: 'VStack', props: { className: 'flex flex-col gap-[16px] w-full' } } },
    { type: 'HStack', label: 'HStack',     icon: '➡', aiRef: 'Empty horizontal stack. Default: gap-[16px] between children, no padding, vertically centered. Children are left-aligned by default — use set_layout({justify:"center"}) to center them horizontally. Call set_spacing to set padding or change the gap.',
      defaultNode: { type: 'HStack', props: { className: 'flex flex-row gap-[16px] w-full items-center' } } },
    { type: 'Center', label: 'Center',     icon: '⊕', aiRef: 'Empty centering container — centers children horizontally always. Vertical centering only works when Center has a defined height. Default height is "fit" (wraps content) — always call set_size({height:"fill"}) when Center is inside a fixed-height parent so it expands and justify-center has space to work.',
      defaultNode: { type: 'Center', props: { className: 'flex flex-col items-center justify-center w-full' } } },
    ...
    { type: 'Box',    label: 'Divider',    icon: '—', aiRef: 'Thin horizontal rule. No children needed. Use set_background to change its color.',
      defaultNode: { type: 'Box', props: { className: 'w-full h-px bg-border' } } },
    { type: 'Box',    label: 'ScrollView', icon: '↕', aiRef: 'Scrollable container. Default max-height 200px — call set_size to increase the scroll area height. Ships with a sample Text child — replace or remove it and add real content.',
  ...
  Buttons: [
    ...
    { type: 'Box', label: 'Icon Btn',        icon: '⬚', aiRef: 'Square icon-only button. Child node: Icon (has its own UUID). To change the icon or color, call set_icon on the Icon child UUID directly — not this wrapper UUID. Default style is ghost/transparent — use set_background on this wrapper to add a fill.',
    ...
    { type: 'Box', label: 'Icon Btn Round',  icon: '◉', aiRef: 'Circular icon-only button. Child node: Icon (has its own UUID). To change the icon or color, call set_icon on the Icon child UUID directly — not this wrapper UUID. Default style is ghost/transparent — use set_background on this wrapper to add a fill.',
  ...
  Media: [
    { type: 'Image',       label: 'Image',        icon: '🖼', aiRef: 'Image element. Add in generate_structure without src — never include a src in the tree. After generate_structure, call search_images then set_src(nodeId, url) to assign the real URL. Resize with set_size. Default: 300×192px, no border-radius. Use set_border to add radius for inline use. No children.',
      defaultNode: { type: 'Image', props: { style: { width: '300px', height: '192px' } }, src: 'https://placehold.co/600x400' } },
    { type: 'Icon', label: 'Icon',          icon: '◈', aiRef: 'Iconify icon. Props: icon (Iconify string like "heroicons:star" or a URL), size (number), color (theme token name e.g. "primary", "muted-foreground", "foreground" — or a hex string like "#3b82f6"). Token names are automatically resolved to var(--theme-*). Use the Settings panel to pick from the icon library or enter a URL. No children.',
    ...
    { type: 'Video',       label: 'Video',         icon: '▶', aiRef: 'Video player. Add in generate_structure without src — never include a src in the tree. Server auto-injects autoPlay/loop/muted/controls/objectFit defaults. After generate_structure, call search_videos then set_src(nodeId, url) to assign the real URL. BACKGROUND LAYER USE: set_size({width:"screen",height:"screen"}) + set_position(id,{position:"absolute",top:0,left:0,zIndex:0}). Default: 400×240, no border-radius. No children.',
