...
const columnSchema = {
  type: 'object',
  properties: {
    name:         { type: 'string', description: 'Column name (camelCase or snake_case).' },
    type:         { type: 'string', enum: COLUMN_TYPE_ENUM as unknown as string[], description: 'Column data type.' },
    required:     { type: 'boolean', description: 'NOT NULL constraint.' },
    unique:       { type: 'boolean', description: 'Unique constraint.' },
    indexed:      { type: 'boolean', description: 'Add a DB index.' },
    defaultVal:   { type: 'string', description: 'Default value SQL expression.' },
    enumValues:   { type: 'array', items: { type: 'string' }, description: 'Allowed values for ENUM columns.' },
  },
  required: ['name', 'type'],
};

const backendTools: BuilderTool[] = [
  // ── Table tools ───────────────────────────────────────────────────────────
  {
    name: 'create_table',
    ...
    input_schema: {
      type: 'object',
      properties: {
        tableName: { type: 'string', description: 'Table name (snake_case, singular).' },
        columns:   { type: 'array', items: columnSchema, description: 'Column definitions.' },
      },
  ...
  },
  {
    ...
    description: TOOL_DESCRIPTIONS['add_table_column'],
    input_schema: {
      type: 'object',
      properties: {
        tableId: { type: 'string', description: 'UUID of the target table.' },
        column:  { ...columnSchema, description: 'Column definition to add.' },
      },
  ...
  {
    name: 'import_erd',
    description: TOOL_DESCRIPTIONS['import_erd'],
    input_schema: {
      type: 'object',
      properties: {
        erd: {
          type: 'string',
          description: 'DBML schema string describing the tables and their relationships. Example: "Table users {\\n  id uuid [pk]\\n  email text [unique, not null]\\n}\\n\\nTable posts {\\n  id uuid [pk]\\n  userId uuid [ref: > users.id]\\n}".',
        },
      },
      required: ['erd'],
    },
  ...
  {
    name: 'read_table',
    description: TOOL_DESCRIPTIONS['read_table'],
    input_schema: {
      type: 'object',
      properties: {
        tableId: { type: 'string', description: 'UUID of the table to read.' },
      },
      required: ['tableId'],
    },
  },
  ...
  {
    name: 'create_server_workflow',
    description: TOOL_DESCRIPTIONS['create_server_workflow'],
    input_schema: {
      type: 'object',
      properties: {
        name:        { type: 'string', description: 'Human-readable workflow name.' },
        kind:        { type: 'string', enum: ['FUNCTION', 'API_ENDPOINT', 'MIDDLEWARE'], description: 'FUNCTION = reusable server fn; API_ENDPOINT = HTTP route; MIDDLEWARE = request interceptor.' },
        description: { type: 'string', description: 'Optional purpose description.' },
        method:      { type: 'string', enum: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE'], description: 'HTTP method (API_ENDPOINT only).' },
        path:        { type: 'string', description: 'URL path, e.g. "/api/products" (API_ENDPOINT only).' },
      },
      required: ['name', 'kind'],
    },
  },
  {
    name: 'add_server_workflow_step',
    description: TOOL_DESCRIPTIONS['add_server_workflow_step'],
    input_schema: {
      type: 'object',
      properties: {
        workflowId: { type: 'string', description: 'UUID of the target server workflow.' },
        step: {
          type: 'object',
          description: 'Step definition. Config values can be plain (string/number) or formula objects { formula: "..." } for dynamic expressions.',
          properties: {
            id:     { type: 'string', description: 'Step ID, e.g. "s1", "s2". Must be unique within the workflow.' },
            name:   { type: 'string', description: 'Human-readable step name.' },
            type:   { type: 'string', description: 'Step type, e.g. tablesInsert, tablesList, tablesGet, tablesUpdate, tablesDelete, hashPassword, verifyPassword, generateToken, verifyToken, sendResponse, throwError, tryCatch, branch, forEach, middlewareNext.' },
            config: {
              type: 'object',
              description: [
                'Step configuration object. Any value can be a plain scalar or { "formula": "..." } for dynamic expressions.',
                'tablesInsert: { table: string, data: FormulaExpr } — data formula must evaluate to a plain object of column→value pairs. NEVER include id, created_at, or updated_at — those are auto-generated. No "values" key.',
                'tablesList:   { table: string, filters?: Array<{ column, operator, value: FormulaExpr }>, pageSize?: number } — use "filters" array, not "filter" object.',
                'tablesGet:    { table: string, rowId: FormulaExpr }',
                'tablesUpdate: { table: string, rowId: FormulaExpr, data: FormulaExpr }',
                'tablesDelete: { table: string, rowId: FormulaExpr }',
                'hashPassword: { password: FormulaExpr } — result: { hash }',
                'verifyPassword: { password: FormulaExpr, hash: FormulaExpr } — result: { match: boolean }',
                'generateToken: { payload: FormulaExpr, expiresIn?: string } — result: { token }. All token fields must be inside payload formula.',
                'verifyToken:  { token: FormulaExpr } — throws on invalid; wrap in tryCatch.',
                'sendResponse: { status: string, bodyType: "JSON", body: FormulaExpr }',
                'throwError:   { message: string, statusCode: number }',
                'tryCatch:     { catchEnabled: true, finallyEnabled: false } — sub-steps go in tryBody/catchBody at step TOP LEVEL, not inside config.',
                'branch:       { condition: FormulaExpr } — sub-steps go in trueBranch/falseBranch at step TOP LEVEL, not inside config.',
                'forEach:      { items: FormulaExpr } — sub-steps go in loopBody at step TOP LEVEL; current item at context.workflow["variables"].item.',
                'middlewareNext: {} — no config fields needed.',
              ].join(' | '),
            },
          },
  ...
  {
    name: 'publish_server_workflow',
    description: TOOL_DESCRIPTIONS['publish_server_workflow'],
    input_schema: {
      type: 'object',
      properties: {
        workflowId: { type: 'string', description: 'UUID of the ENDPOINT workflow to publish.' },
      },
      required: ['workflowId'],
    },
  },
  {
    name: 'read_workflow',
    description: TOOL_DESCRIPTIONS['read_workflow'],
    input_schema: {
      type: 'object',
      properties: {
        workflowId: { type: 'string', description: 'UUID of the server workflow to read.' },
      },
      required: ['workflowId'],
    },
  },
...
/** Backend Agent — creates tables, server workflows (ENDPOINT/FUNCTION/MIDDLEWARE), and adds backend steps.
 *  All tool calls are executed server-side in the API route; no Zustand store involvement. */
export const BACKEND_AGENT_TOOLS: BuilderTool[] = [...backendTools];
