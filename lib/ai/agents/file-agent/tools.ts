...
export const FILE_AGENT_TOOLS: FileTool[] = [
  ...
  {
    name: 'write_page',
    description: 'Create or fully replace a page file at pages/<Name>/page. Use this instead of write_file for ALL page writes. The ui array is fully typed — StyleParams enums enforce correct direction/weight/text values automatically.\n\nGroup strategy: small/medium page (1–2 major sections) → one write_page with all content. Large page (3+ major sections) → call write_page first with thin _group placeholder stubs (just { "type": "Box", "_group": "SectionName" }), then call write_group once per section with the full content. Surgical node edit → edit_page. Section rewrite → write_group. After write_group, do NOT rewrite the full page — the group merges automatically.',
    input_schema: {
      type: 'object',
      $defs: {
        StyleParams: {
        ...
        UINode: {
          ...
          properties: {
            ...
            _group:     { type: 'string', description: 'Named section label.' },
  ...
  {
    name: 'write_group',
    description: 'Write a named _group section within an existing page. Replaces the _group node in-place (or appends if not yet present). Use for large pages: call write_page first with thin _group placeholder stubs, then write_group once per section with the full content.',
    input_schema: {
      type: 'object',
      $defs: {
        StyleParams: {
          ...
          properties: {
            ...
            inset0: { type: 'boolean' },
            top: { type: 'number' }, right: { type: 'number' }, bottom: { type: 'number' }, left: { type: 'number' },
            z: { type: 'number' }, overflow: { type: 'string', enum: ['hidden','auto','visible','scroll','x-auto','y-auto'] },
            cursor: { type: 'string', enum: ['auto','default','pointer','not-allowed','grab','move','text','crosshair'] },
            opacity: { type: 'number' }, objectFit: { type: 'string', enum: ['cover','contain','fill','none'] },
            extra: { type: 'string' },
          },
          ...
          properties: {
            ...
            props: {
              ...
              properties: {
                ...
                loop: { type: 'boolean' }, autoPlay: { type: 'boolean' },
                muted: { type: 'boolean' }, controls: { type: 'boolean' },
                placeholder: { type: 'string' }, type_: { type: 'string' },
              },
            },
            actions: {
              type: 'array',
      ...
      properties: {
        page:  { type: 'string', description: 'Page name. Handler writes to pages/<page>/groups/<group>.' },
        group: { type: 'string', description: 'Group section name matching a _group annotation.' },
        ui:    { type: 'array', items: { type: 'object' }, description: 'Full content for this section. Pass as a JSON array literal — never as a JSON string.' },
      },
      required: ['page', 'group', 'ui'],
    } as unknown as { type: 'object'; properties: Record<string, unknown>; required?: string[] },
  },
