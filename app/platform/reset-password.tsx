...
export default function ResetPasswordPage() {
  ...
  if (!token) {
    return (
      <div style={{ textAlign: 'center', color: '#f87171', padding: 40 }}>
        Invalid reset link. <Link to="/forgot-password" style={{ color: '#60a5fa' }}>Request a new one.</Link>
      </div>
    );
  }
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (getStrength(password) < 4) { setError('Please satisfy all password requirements.'); return; }
    setError('');
    setLoading(true);
    try {
