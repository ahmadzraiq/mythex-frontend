...
function AutocompleteDropdown({ items, activeIdx, onSelect, onClose, anchorPos, triggerMode }: DropdownProps) {
  ...
  return createPortal(
    ...
    >
      {/* Header */}
      <div style={{ padding: '4px 10px', borderBottom: '1px solid #1f2937', fontSize: 9, color: 'var(--bld-text-disabled)', background: 'var(--bld-bg-canvas)', flexShrink: 0 }}>
        ...
        {Object.entries(grouped).map(([group, groupItems]) => (
          <div key={group}>
            {Object.keys(grouped).length > 1 && (
              <div style={{ padding: '3px 10px 1px', fontSize: 9, color: 'var(--bld-text-disabled)', background: 'var(--bld-bg-canvas)', textTransform: 'none' }}>
                {group}
              </div>
            )}
            {groupItems.map(item => {
              ...
              return (
                ...
                >
                  ...
                  <span style={{ fontSize: 10, color: isActive ? '#93c5fd' : 'var(--bld-text-3)', flexShrink: 0, maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {item.description}
                  </span>
                </div>
              );
