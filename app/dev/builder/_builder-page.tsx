...
function TopBar({
  ...
  // Deployed live URL — single-level subdomains covered by Cloudflare Universal SSL
  const _builderHost = typeof window !== 'undefined' ? window.location.host : '';
  const liveUrl = _builderHost === 'staging.app.mythex.ai'
    ? `https://${projectId}-staging.mythex.ai`
    : _builderHost === 'app.mythex.ai'
      ? `https://${projectId}-app.mythex.ai`
      : `http://${projectId}-app.localhost:3001`;
  ...
  useEffect(() => {
    ...
    fetch(`${BACKEND_BASE}/v1/projects/${projectId}`, { credentials: 'include' })
      .then(r => r.json())
      ...
      .catch(() => {});
    fetch(`${BACKEND_BASE}/v1/projects/${projectId}/versions`, { credentials: 'include' })
      .then(r => r.json())
  ...
  async function handleDeploy() {
    ...
    try {
      const res = await fetch(`${BACKEND_BASE}/v1/projects/${projectId}/${endpoint}`, {
        method: 'POST',
  ...
  async function handleRestore(versionId: string) {
    ...
    setDeployError('');
    try {
      const res = await fetch(`${BACKEND_BASE}/v1/projects/${projectId}/versions/${versionId}/restore`, { method: 'POST', credentials: 'include' });
  ...
  async function handleSaveDomain() {
    ...
    setVerifyMsg('');
    try {
      const res = await fetch(`${BACKEND_BASE}/v1/projects/${projectId}/custom-domain`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleVerifyDomain() {
    ...
    setVerifyMsg('');
    try {
      const res = await fetch(`${BACKEND_BASE}/v1/projects/${projectId}/custom-domain/verify`, { method: 'POST', credentials: 'include' });
  ...
  async function handleRemoveDomain() {
    if (!projectId) return;
    await fetch(`${BACKEND_BASE}/v1/projects/${projectId}/custom-domain`, { method: 'DELETE', credentials: 'include' });
    setCustomDomain('');
    setDomainInput('');
    setCustomDomainVerified(false);
    setVerifyMsg('');
...
function BuilderPageInner() {
  ...
  useEffect(() => {
    ...
    const handler = () => {
      ...
      import('@/lib/builder/autosave').then(({ serializeBuilderState }) => {
        ...
        fetch(`${BACKEND_BASE}/v1/projects/${projectId}/config`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
  ...
  const openPreview = useCallback(async () => {
    ...
    if (projectId) {
      ...
      let previewToken = '';
      try {
        const tokenRes = await fetch(`${BACKEND_BASE}/v1/projects/${projectId}/preview-token`, {
          method: 'POST',
          credentials: 'include',
        });
        if (tokenRes.ok) {
