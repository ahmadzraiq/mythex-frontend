...
type TabId = 'navbar' | 'layout' | 'palettes' | 'fonts' | 'variants';

const TABS: { id: TabId; generator: string; label: string }[] = [
  { id: 'navbar', generator: 'navbar', label: 'Navbar' },
];
...
export function AiResponsePreviewOverlay() {
  ...
  const generator = TABS.find((t) => t.id === activeTab)?.generator ?? '';
  ...
  useEffect(() => {
    ...
    fetch(`/api/ai-responses?generator=${generator}${fileParam}`)
      .then((r) => r.json())
      .then((data) => {
        ...
        if (output.screen && typeof output.screen === 'object') {
          ...
          const layout: LayoutSchema = {
            ...
            layoutParts: (typeof structure === 'object' && structure
              ...
              : undefined) as LayoutSchema['layoutParts'],
          };
  ...
  }, [activeTab, selected, setGenerated, pathname, router]);
              ...
              {loading ? (
                ...
                entries.map((e, i) => (
                  <button
                    ...
                    className={`w-full text-left px-2 py-1.5 rounded text-xs truncate mb-1 ${
                      selected === e
                        ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300'
                        : 'hover:bg-gray-100 dark:hover:bg-gray-800'
                    }`}
                  >
                    {inputSummary(e.input)}
                    {e.evalResult && (
                      <span className={`block text-[10px] ${e.evalResult === 'PASS' ? 'text-green-600' : 'text-red-600'}`}>
                        {e.evalResult}
                      </span>
                    )}
                  </button>
